...
/// @DnDVersion : 1
/// @DnDHash : 74C028A6
/// @DnDArgument : "code" "///@desc get input$(13_10)if (hp > 0) {$(13_10)	if (room == rmMageHelp) {$(13_10)		//do nothing$(13_10)	} else if (auto || ai) {$(13_10)		scrAIMageGetInput()$(13_10)	} else {$(13_10)		scrPlayerGetInput()$(13_10)	}$(13_10)}"
///@desc get input
if (hp > 0) {
	if (room == rmMageHelp) {
		//do nothing
...
for (var i = 0; i < numAbilities; i++) {
	...
	if (durationHeld[index] > 0 && !heldBefore[index]) {
		if (aState != abilityKey[i] && aPhase == "n") {
			...
			//online stuff
			clientAState = aState;
			timeToActivate = current_time + windUp[i] * 1000 / 60
			abilitySentNum++
		}
...
if (aState == "1") {
	...
	if (aPhase == "d") {
		if (timeInAPhase >= 0 && firstTime[1]) {
			...
			audio_play_sound(sndFireLob, 1, false)
			firstTime[1] = false
		}
	...
	if (aPhase == "d") {
		if (timeInAPhase >= 0 && firstTime[3]) {
			...
			ds_list_add(envMoveModList, moveMod)
			audio_play_sound(sndBlast, 1, false)
			firstTime[3] = false
		}
	}
