...
/// @DnDVersion : 1
/// @DnDHash : 74C028A6
/// @DnDArgument : "code" "///@desc get input$(13_10)if (hp > 0) {$(13_10)	if (room == rmRogueHelp) {$(13_10)	} else if (auto || ai) {$(13_10)		scrAIRogueGetInput();$(13_10)	} else {$(13_10)		scrPlayerGetInput();$(13_10)	}$(13_10)}"
///@desc get input
if (hp > 0) {
	if (room == rmRogueHelp) {
	} else if (auto || ai) {
...
if (aState == "1") {
	...
	if (aPhase == "d" && timeInAPhase >= 0 && firstTime[1]) {
		...
		audio_play_sound(sndRogueDash, 1, false)
		firstTime[1] = false;
	}
	...
	if (aPhase == "d") {
		if (timeInAPhase >= 0 && firstTime[3]) {
			for (var i = 0; i < blinkNumBlades; i++) {
				blinkCurrAngle = 90 - 90 * image_xscale - blinkBladeAngle / 2 + blinkBladeAngle * i / (blinkNumBlades - 1);
			...
			moveMod.duration = duration[1];
			ds_list_add(envMoveModList, moveMod);
			audio_play_sound(sndRend, 1, false)
			firstTime[3] = false;
		}
