...
if (!lockedIn[thisNumber] && thisInControl[thisNumber]) {
	...
	for (var i = 4; i < 8; i++) {
		if (durationHeld[i] > 0 && !heldBefore[i]) {
			...
			show_debug_message(string(i) + " " + string(durationHeld[i]))
			image_index = 1
			break
		}
	}
